...
class Feed extends React.Component {
  ...
  handleSubmit = (event) => {
    ...
    alert('An essay was submitted: ' + this.state.statusValue)
    ...
    return (
      <div className="card m-2" style={{ marginBottom: '2rem' }}>
        <div className="card-body">
          ...
          <p className="card-text text-muted text-right">Publié par <Link to={'/profile/' + this.props.post.author._id}>{this.props.post.author.firstName} {this.props.post.author.lastName}</Link> <Moment locale="fr"
          ...
          <button className="btn btn-primary mr-2" onClick={this.like} disabled={this.state.buttonLikeDisabled}>{ this.state.iLiked ? '👎 Je n\'aime plus' : '👍 J\'aime' }</button>
          <button className="btn btn-primary" onClick={this.showComments} disabled={this.state.buttonCommentsDisabled}>{this.props.post.comments.length > 0
            ? this.props.post.comments.length > 1 ? 'Afficher ' + this.props.post.comments.length + ' commentaires' : 'Afficher 1 commentaire'
            : 'Aucun commentaire'}</button>
          <div className="comments">{this.state.comments.map((comment, i) => {
            return (
              <div className="bg-light m-2 p-2" key={i}>
                <img className="rounded-circle" src={comment.author.profilePic} alt="Photo de profil" style={{maxWidth: '5rem'}} />
                <p>{comment.content}</p>
                <p className="text-muted">Publié par <Link to={'/profile/' + comment.author._id}>{comment.author.firstName} {comment.author.lastName}</Link> <Moment locale="fr"
